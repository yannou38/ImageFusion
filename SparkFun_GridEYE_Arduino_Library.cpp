...
void GridEYE::begin(uint8_t deviceAddress)
{
	_deviceAddress = deviceAddress;
	deviceHandle = wiringPiI2CSetup(_deviceAddress);
}
...
void GridEYE::setRegister(unsigned char reg, unsigned char val)
{
	...
	wiringPiI2CWriteReg8(deviceHandle,reg,val);
...
int16_t GridEYE::getRegister(unsigned char reg, int8_t len)
{
	...
	_i2cPort->endTransmission();*/
	result = wiringPiI2CReadReg16(deviceHandle,reg);
	return result;
